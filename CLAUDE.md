...
| **Document (PDF/DOCX) capture** | `services/message_processor.py` → `process()` document branch + `_load_stored_bytes()`. When `message_type == "document"` and `media_url`, loads the stored bytes straight from the `StoredImage` row (the `/api/images/{id}` route is auth-gated, so an HTTP fetch would 401), runs `document_processor.extract_text_from_bytes()` (magic-byte PDF/DOCX detection), folds the **full text into `content`** (keyword-searchable, capped 10k) and **saves directly as a Remember note (early `return`) — it never reaches intent parsing or list detection** (a PDF's many short lines would otherwise be mis-detected as a 500-item list). Embedding uses only a **focused lead** (`embed_text` = caption + first ~1.5k chars) via the `embed_text` param on `_save_single` — embedding the whole document averages many pages into a diffuse vector that under-scores the 0.40 similarity floor. **Extracted text is sanitized** (`document_processor._sanitize`) to strip `0x00`/control bytes — Postgres rejects `0x00` in UTF-8 text columns (asyncpg `CharacterNotInRepertoireError`), which previously 500'd the capture. The capture route (`main.py`) also folds the filename into `content` (so `"pdf"`/filename keyword-match) and stamps `tags.file_name` + `is_document`. |
